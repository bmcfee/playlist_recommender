...
\begin{align}
\nabla_b &= \sum_{i\in\U}\sum_{y\in\Y_i}\sum_{t=0}^T \left( \e_t - \sum_{e\in\E} p_e^t \sum_{k\in e} \e_k \frac{\exp\{u_i\trans v_k + b\trans \e_k\}}{\sum_{j\in e} \exp\{u_i\trans
v_j + b\trans \e_j\}}  \right)\notag\\
&= \sum_{i\in\U}\sum_{y\in\Y_i}\sum_{t=0}^T \left( \e_t - \sum_{e\in\E} p_e^t \frac{H_{\cdot, e} \odot \exp\{u_i\trans V + b \}}{ H_{\cdot,e}\trans \exp\{u_i\trans V + b\}}  \right)
...
\begin{equation}
\phi(w) \defeq \sum_{i\in\U}\sum_{y\in\Y_i}\sum_{t=0}^T \left(\sum_{e} \frac{H_{t-1,e} H_{t,e} q_e }{ (H_{t-1,\cdot} \odot H_{t, \cdot})\trans q }w\trans \e_e - \log
H_{t-1,\cdot}\trans \exp\{w\}\right), \label{eq:wstep:surrogate}
\end{equation}
