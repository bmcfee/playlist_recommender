...
&= u_i\trans v_0 + \log \sum_{e\in \E}\frac{ H_{0, e}\exp\{w_e\}}{H_{\cdot, e}\trans \exp\{u_i\trans V + b \}}\label{eq:ustep:initial}
\end{align}
...
&= u_i\trans v_t + \log\sum_{e\in \E} \frac{H_{t-1,e} H_{t, e} \exp\{w_e\} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} },\label{eq:ustep:bigram}
\end{align}
...
\Cref{eq:ustep:obj} is a sum over terms of the form of \cref{eq:ustep:bigram}, which are not concave.  However, we can instead maximize a variational lower bound:
\begin{align}
u_i\trans v_t + \log\sum_{e\in \E} \frac{H_{t-1,e} H_{t, e} \exp\{w_e\} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } 
&= u_i\trans v_t + \log\frac{1}{|\E|}\sum_{e\in \E} \frac{H_{t-1,e} H_{t, e} \exp\{w_e\} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } + \ccancel[red]{\log|\E|}\notag\\
&\geq u_i\trans v_t + \frac{1}{|\E|}\sum_{e\in \E} \log \frac{H_{t-1,e} H_{t, e} \exp\{w_e\} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } &\text{(Jensen's inequality)} \notag\\
&= u_i\trans v_t + \frac{1}{|\E|}\sum_{e\in \E} \ccancel[red]{\log H_{t-1,e} H_{t, e} \exp\{w_e\}} - \log{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } \notag\\
&= u_i\trans v_t - \frac{1}{|\E|}\sum_{e\in \E} \log{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } \label{eq:ustep:concave}
\end{align}
The second term in \cref{eq:ustep:concave} is a sum over negative log-sum-exp terms, and is therefore concave in $u_i$.  What is lost in this approximation is the dependence on the
edge weights $w_e$, and the bigram relation terms.  In effect, the approximation tries to push $u_i$ towards all observed song selections $x_t$, and away from all other songs.

Note that applying the same approximation to \cref{eq:ustep:initial} results in the same form as \cref{eq:ustep:concave}, as all bigram dependencies have been broken.
Combining terms, we achieve the lower-bound approximation:
\begin{equation}
f(u) \gtrsim \phi(u) = \sum_{y \in \Y_i} \left(\sum_{t=0}^T u_i\trans v_t - \frac{1}{|\E|} \sum_{e \in \E} \log H_{\cdot, e}\trans \exp\{u_i\trans V + b\} \right)
\end{equation}
