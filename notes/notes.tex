...
&= u_i\trans v_0 + \log \sum_{e\in \E}\frac{\exp\{w_e\}}{\sum_f \exp\{w_f\}}\frac{ H_{0, e}}{H_{\cdot, e}\trans \exp\{u_i\trans V + b \}}\label{eq:ustep:initial}
\end{align}
...
&= \log\sum_{e\in \E} \frac{H_{t-1,e} \exp\{w_e\}}{H_{t-1, \cdot} \exp\{w\} } \frac{ H_{t,e} \exp\{u_i\trans v_t + b_t \} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} }\notag\\
...
the summation:\footnote{For this approximation to be well-defined, we follow the convention $\log\nicefrac{0}{0} = 0$.}
% \begin{align}
% u_i\trans v_t + \log\sum_{e\in \E} \frac{H_{t-1,e} \exp\{w_e\}}{H_{t-1,\cdot} \exp\{w\} }\frac{ H_{t,e} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } &\geq
% u_i\trans v_t + \sum_{e\in \E} \frac{H_{t-1,e} \exp\{w_e\}}{H_{t-1,\cdot} \exp\{w\} }\log\frac{ H_{t,e} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } \label{eq:ustep:concave}
% \end{align}
\begin{align}
u_i\trans v_t + \log\sum_{e\in \E} \exp\{w_e\} \frac{H_{t-1,e} H_{t, e} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } 
&= u_i\trans v_t + \log \sum_{e\in \E}\frac{\exp\{w_e\}}{\sum_{f} \exp\{w_f\}} \frac{H_{t-1,e} H_{t, e} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } + \ccancel[red]{\log\sum_f
\exp\{w_f\}}\notag\\
&\geq u_i\trans v_t + \sum_{e\in \E} \frac{\exp\{w_e\} }{\sum_f \exp\{w_f\} }\log \frac{H_{t-1,e} H_{t, e} }{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } \notag\\
&= u_i\trans v_t + \sum_{e\in \E} \frac{\exp\{w_e\}}{\sum_f \exp\{w_f\}} \left(\ccancel[red]{\log H_{t-1,e} H_{t, e} } - \log{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} }
\right)\notag\\
&= u_i\trans v_t - \sum_{e\in \E} \frac{\exp\{w_e\}}{\sum_f \exp\{w_f\}} \log{H_{\cdot,e}\trans \exp\{u_i\trans V + b \} } \label{eq:ustep:concave}
\end{align}
The second term in \cref{eq:ustep:concave} is a convex combination of negative log-sum-exp terms, and is therefore concave in $u_i$.  
What is lost in this approximation is the dependence on bigram transitions.

Note that applying the same approximation to \cref{eq:ustep:initial} results in the same form as \cref{eq:ustep:concave}.
Combining terms, we achieve the lower-bound approximation:
\begin{equation}
f(u) \gtrsim \phi(u) = \sum_{y \in \Y_i} \left(\sum_{t=0}^T u_i\trans v_t - \sum_{e \in \E} \frac{\exp\{w_e\}}{\sum_f \exp\{w_f\}} \log H_{\cdot, e}\trans \exp\{u_i\trans V + b\} \right)
\end{equation}
